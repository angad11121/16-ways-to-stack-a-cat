...
void push_stack(stack_id id, long long value)
{
...
long long pop_stack(stack_id id)
{
    // Check for non-positive ids
    if(id <= 0) return LONG_LONG_MIN;
    ...
    // Out-of-Bounds Check
    if(count!=id) return LONG_LONG_MIN;
    // Existance Check
...
void destroy_stack(stack_id id)
{
    // Check for non-positive ids
    if(id <= 0) return;
